...
| **🖼️ スプレーアート・ロゴ** | アイコンを使わず「Vandal」の文字だけで見せる、大胆でかっこいいスプレーアート風ワードマーク（ライト / ダーク対応） |
...
5. **プライバシー** — 訪問者には URL を省略表記でしか見せません（内部の共同作業には完全な URL を使用）。管理者 API または `data/config.json` の `"meshPrivate": true`（env: `VANDAL_MESH_PRIVATE=1`）で**プライベートインスタンス**（URL を一切共有しない匿名参加）に切替可能。利用者向け設定にはメッシュ参加を変更するボタンを表示せず、内部の共同作業にはデプロイされた生きている全インスタンスが適用されます。
